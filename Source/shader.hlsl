...
float4 PS(VertexOut pin) : SV_Target
{
    ...
    float4 PointLight =pointAttenuation * pointLightColour * (pointDiffuse);
    
    float4 finalColour = saturate((diffuse + ambient + PointLight + finalSpecularColor) * materialColour);

    // returning the final colour produced
    return finalColour;
}